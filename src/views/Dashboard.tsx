...
function ResumeLab({ onBack, selectedRole, fetchProfile }: { onBack: () => void; selectedRole: string, fetchProfile: () => Promise<void> }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
      <button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white transition-all uppercase text-[10px] font-black tracking-widest">
      ...
      {!result ? (
        <div className="p-20 border-2 border-dashed border-white/10 rounded-[3.5rem] bg-white/[0.02] flex flex-col items-center justify-center text-center">
          ...
          <label htmlFor="resume-input" className={`px-10 py-4 bg-white text-black rounded-full font-bold cursor-pointer hover:scale-105 transition-all text-[11px] uppercase tracking-[0.2em] ${analyzing ? 'opacity-50 cursor-not-allowed' : ''}`}>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <button onClick={() => setResult(null)} className="col-span-12 text-center text-gray-600 hover:text-white transition-colors text-[10px] font-black uppercase tracking-[0.4em]">
...
export default function Dashboard({ onNewCall }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        if (deleteModal.type === 'interview') {
          ...
          setUserData({ ...userData, resumes: userData.resumes.filter((s: any) => s.id !== deleteModal.id) });
        }
      }
  ...
  if (selectedReport) {
    return (
      <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="min-h-screen bg-[#050505] text-white p-6 md:p-12 overflow-y-auto custom-scrollbar">
        <div className="max-w-5xl mx-auto space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <button onClick={() => setSelectedReport(null)} className="group flex items-center gap-2 text-gray-500 hover:text-white transition-all uppercase text-[10px] font-black tracking-[0.3em]">
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 lg:col-span-4 p-10 rounded-[3rem] bg-indigo-600 shadow-xl shadow-indigo-500/10 transition-all hover:scale-[1.01]">
               ...
               <div className="text-8xl font-bold mt-4 tracking-tighter italic drop-shadow-2xl">{selectedReport.score}<span className="text-3xl not-italic opacity-40">%</span></div>
            ...
            <div className="col-span-12 lg:col-span-8 p-10 rounded-[3rem] bg-[#0A0A0A] border border-white/10 backdrop-blur-3xl flex flex-col justify-center relative">
               ...
               <h3 className="text-2xl md:text-4xl font-bold italic tracking-tight text-white leading-[1.15]">"{selectedReport.verdict}"</h3>
            </div>
          </div>
        </div>
      </motion.div>
  ...
  if (selectedResume) {
    return (
      <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="min-h-screen bg-[#050505] text-white p-6 md:p-12 overflow-y-auto custom-scrollbar">
        <div className="max-w-5xl mx-auto space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <button onClick={() => setSelectedResume(null)} className="group flex items-center gap-2 text-gray-500 hover:text-white transition-all uppercase text-[10px] font-black tracking-[0.3em]">
              <ChevronLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to History
            </button>
            <div className="flex items-center gap-4 px-5 py-2 rounded-2xl bg-white/3 border border-white/5">
              <FileText size={14} className="text-purple-500" />
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">{selectedResume.fileName}</span>
              <div className="w-px h-3 bg-white/10" />
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 lg:col-span-4 p-12 rounded-[3.5rem] bg-purple-600 shadow-2xl shadow-purple-500/20">
               ...
               <div className="text-8xl font-bold mt-4 tracking-tighter italic">{selectedResume.score}%</div>
            </div>
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-white overflow-hidden font-sans selection:bg-indigo-500/30">
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          <header className="flex justify-between items-center mb-4">
            <div className="relative group w-full max-w-md hidden md:block">
              ...
              <input type="text" placeholder="Search sessions..." className="w-full bg-[#111] border border-white/5 rounded-xl py-3 pl-12 pr-4 text-xs focus:outline-none focus:border-indigo-500/50 transition-all" />
            </div>
            <div className="flex items-center gap-4 ml-auto">
              <div className="p-2 bg-[#111] rounded-lg border border-white/5 text-gray-400 cursor-pointer hover:text-white transition-colors relative"><Bell size={18} /><span className="absolute top-2 right-2 w-1.5 h-1.5 bg-indigo-500 rounded-full border-2 border-black" /></div>
              <div className="flex items-center gap-3 pl-4 border-l border-white/5">
                <div className="text-right hidden sm:block"><div className="text-xs font-bold">{userData?.name || 'Developer'}</div><div className="text-[9px] text-gray-500 uppercase tracking-widest">Free Plan</div></div>
            ...
            {activeTab === 'dashboard' ? (
              <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                <section className="relative p-8 md:p-12 rounded-[2.5rem] bg-linear-to-br from-indigo-900/20 via-transparent to-transparent border border-white/10 overflow-hidden shadow-2xl">
                  <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="text-center md:text-left">
                      <h2 className="text-3xl md:text-5xl font-bold text-white mb-3 tracking-tighter italic">Welcome back, {userData?.name?.split(' ')[0]}</h2>
                      <p className="text-gray-400 max-w-md text-sm font-light leading-relaxed">Your readiness score is looking sharp for <span className="text-white font-medium">{selectedRole}</span> roles.</p>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                      <div className="bg-black/40 backdrop-blur-md p-2 rounded-2xl border border-white/5 flex items-center gap-4">
                         <select value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)} className="bg-transparent text-[10px] font-black uppercase outline-none text-white cursor-pointer px-4">
                ...
                <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <QuickActionCard icon={<Mic className="text-indigo-400" />} title="Start Interview" description="Live audio simulation" onClick={() => onNewCall(selectedRole)} highlight />
                  <QuickActionCard icon={<FileCheck className="text-purple-400" />} title="Resume Prep" description="Assessment & Grade" onClick={() => setActiveTab('resume')} />
                  <QuickActionCard icon={<Building2 className="text-purple-400" />} title="Company Prep" description="AI Candidate Evaluation" onClick={() => {}} />
                </section>

                <div className="grid grid-cols-12 gap-6">
                  <div className="col-span-12 lg:col-span-8 bg-[#111] border border-white/5 rounded-[2.5rem] p-8">
                    <div className="h-62.5 w-full min-h-0"><ResponsiveContainer width="100%" aspect={3}><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="#ffffff05" vertical={false} /><XAxis dataKey="date" stroke="#333" fontSize={10} tickLine={false} axisLine={false} /><YAxis domain={[0, 100]} hide /><Tooltip contentStyle={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '12px' }} /><Line type="monotone" dataKey="score" stroke="#6366f1" strokeWidth={3} dot={{ r: 4, fill: '#6366f1' }} /></LineChart></ResponsiveContainer></div>
                  </div>
                  <div className="col-span-12 lg:col-span-4 space-y-6">
                    <div className="bg-[#111] border border-white/5 rounded-[2.5rem] p-8"><h4 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 mb-6"><Sparkles size={14} className="text-indigo-400" /> Career Readiness</h4><div className="space-y-6"><ProgressStat label="Technical Depth" value={78} /><ProgressStat label="Communication" value={92} /><ProgressStat label="Logical Reasoning" value={64} /></div></div>
                  </div>
                </div>

                <div className="col-span-12 space-y-4">
                  <h4 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 px-4"><History size={14} /> Session Log</h4>
                  ...
                  {interviews.length > 4 && (
                    <button onClick={() => setActiveTab('history')} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors border border-dashed border-white/10 rounded-2xl">View All Sessions</button>
                  )}
                </div>
              </motion.div>
            ) : activeTab === 'resume' ? (
              ...
              <motion.div key="history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-12 pb-20">
                <div className="flex items-center justify-between">
                  <h2 className="text-3xl font-bold italic tracking-tighter">Your Archives</h2>
                <button onClick={() => setActiveTab('dashboard')} className="text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white">Close History</button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      )) : <p className="text-gray-600 italic text-sm px-4">No interviews yet.</p>}
                    </div>
                  </div>
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      )) : <p className="text-gray-600 italic text-sm px-4">No resumes yet.</p>}
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <AnimatePresence>{deleteModal.isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setDeleteModal({ isOpen: false, id: null })} className="absolute inset-0 bg-black/80 backdrop-blur-md" /><motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative bg-[#111] border border-white/10 p-10 rounded-[2.5rem] max-w-sm w-full text-center shadow-2xl"><AlertCircle size={40} className="mx-auto text-red-500 mb-6" /><h3 className="text-xl font-bold mb-2">Delete {deleteModal.type === 'interview' ? 'Session' : 'Resume'}?</h3><div className="flex gap-4"><button onClick={() => setDeleteModal({ isOpen: false, id: null })} className="flex-1 py-3 rounded-xl bg-white/5 text-[10px] font-black uppercase">Cancel</button><button onClick={confirmDelete} className="flex-1 py-3 rounded-xl bg-red-600 text-[10px] font-black uppercase">Delete</button></div></motion.div></div>
      )}</AnimatePresence>
    </div>
  );
}
...
function HistoryItem({ data, onClick, onDelete, type }: any) {
  ...
  return (
    <motion.div onClick={onClick} whileHover={{ x: 4, backgroundColor: "rgba(255,255,255,0.03)" }} className="p-5 rounded-4xl bg-[#111] border border-white/5 flex items-center justify-between group cursor-pointer transition-all">
      <div className="flex items-center gap-5">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold italic transition-all ${isInterview ? 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white' : 'bg-purple-500/10 border border-purple-500/20 text-purple-400 group-hover:bg-purple-600 group-hover:text-white'}`}>
          {data.score}%
        </div>
        <div>
          <h5 className="font-bold text-sm tracking-tight text-white">{isInterview ? (data.verdict || "Session") : (data.fileName || "Resume")}</h5>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest italic">{data.role}</span>
            <span className="text-[12px] text-gray-800">/</span>
            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">{new Date(data.date).toLocaleDateString()}</span>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2">
        <button onClick={onDelete} className="p-3 text-gray-700 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
...
function QuickActionCard({ icon, title, description, onClick, highlight = false }: any) {
  return (
    <motion.div whileHover={{ y: -4, backgroundColor: "rgba(255,255,255,0.04)" }} whileTap={{ scale: 0.98 }} onClick={onClick} className={`cursor-pointer p-6 rounded-4xl border transition-all flex items-center gap-5 ${highlight ? 'bg-indigo-600/5 border-indigo-500/20' : 'bg-[#111] border-white/5'}`}><div className="p-4 rounded-2xl bg-white/5">{icon}</div><div><h5 className="font-bold text-sm text-white">{title}</h5><p className="text-[10px] text-gray-500 font-medium uppercase">{description}</p></div><ChevronRight size={16} className="ml-auto text-gray-700" /></motion.div>
  );
}

function SidebarLink({ icon, label, active = false, onClick }: { icon: any, label: string, active?: boolean, onClick?: () => void }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${active ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/10' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>{icon}<span className="text-[11px] font-bold tracking-tight">{label}</span></div>
  );
}

function ProgressStat({ label, value }: { label: string, value: number }) {
  return (
    <div><div className="flex justify-between items-center mb-2"><span className="text-[10px] font-bold text-gray-400 tracking-tight">{label}</span><span className="text-[10px] font-black text-indigo-400">{value}%</span></div><div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${value}%` }} className="h-full bg-indigo-500" /></div></div>
  );
}